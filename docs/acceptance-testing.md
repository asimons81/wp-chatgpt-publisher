...
Vercel build `dpl_A3Gwfwq5uiJ35JxM6L571YESiBsD` is READY with a production-only Neon `free_v3` database. `/healthz` and `/readyz` return 200, version `1.0.2` is public, OAuth metadata is healthy, the UI asset is cacheable with security headers, unauthenticated `/mcp` returns the RFC 9728 challenge, and malformed JSON returns a safe 400 validation response. A production log probe confirms that OAuth/query credentials are omitted while the sanitized route is retained. ChatGPT developer-mode desktop acceptance against a disposable HTTPS WordPress site passes, including OAuth, read-only discovery, a safe draft, and a publish review that does not execute. See [the dated acceptance record](acceptance-chatgpt-2026-07-13.md). The automated real-MCP E2E passes every remaining editorial operation; repeating those operations manually inside ChatGPT requires the owner to complete the OpenAI developer identity prompt first.
