...
Snapshot PostgreSQL and WordPress, deploy the new service, wait for readiness, run smoke tests, then drain the old version. Service migrations must remain forward-compatible for one release. WordPress performs idempotent schema upgrades on load, preserves tables on deactivation/default uninstall, and removes them only when `WPCP_REMOVE_DATA_ON_UNINSTALL` is explicitly `true`. Keep the prior image and ZIP until the post-upgrade acceptance pass is complete.
